...
interface EventModalProps {
  date: Date;
  events: { event1: string[]; event2: string[] };
  onClose: () => void;
  onAddEvent: (eventType: string, text: string) => void;
  onUpdateEvent: (eventType: string, index: number, newText: string) => void;
  onDeleteEvent: (eventType: string, index: number) => void;
  monthNames: string[];
}
...
const EventModal: React.FC<EventModalProps> = ({ 
  ...
  const [editingEvent, setEditingEvent] = useState<{ type: string; index: number; text: string } | null>(null);

  const handleSubmit = (eventType: string, e: React.FormEvent) => {
    e.preventDefault();
  ...
  const handleEdit = (eventType: string, index: number, text: string) => {
    setEditingEvent({ type: eventType, index, text });
  };

  const handleSaveEdit = (eventType: string, index: number) => {
  ...
  const handleEditChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setEditingEvent(prev => prev ? { ...prev, text: e.target.value } : null);
  };

  const renderEventList = (eventType: string, events: string[]) => {
